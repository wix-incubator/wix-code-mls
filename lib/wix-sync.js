...
export async function uploadImage(imageData, mimeType, resourceID, index, id, siteAPI) {
  let data = {
    mimeType: mimeType,
    resource: resourceID,
    id: id
  ...
  const {uploadUrl, uploadToken} = result.data;
  //upload the image
  return await uploadImageViaUploadUrl(uploadUrl, uploadToken, imageData, `${resourceID}-${id}-${index}.jpg`, mimeType);
}
