...
export async function saveItemBatch(items, siteAPI, wixCollection) {
  ...
  if (result.data.status === 'ok')
    return result.data.result; // bulkSave result
  else
    throw new Error('failed to call [saveItemBatch] with error - ' + result.stack)
}
...
export async function batchCheckUpdateState(items, siteAPI, wixCollection) {
  ...
  if (result.data.status === 'ok')
    return result.data.result; // {status: not-found | ok | need-update, id: string}[]
  else
    throw new Error('failed to call [batchCheckUpdateState] with error - ' + result.stack)
}
...
export async function uploadImages(imagesData, resourceID, resourceKey, siteAPI) {
  ...
  let startUpload = new Date();
  let result = await withRetry(
    () => post(siteAPI.getImageUploadUrl, payload), shouldRetry);

  let uploadUrls = result.data;
//  const uploadUrlsAndData = zip(imagesData, uploadUrls);
  ...
  await Queue(1, imagesData.map((item, index) => {
    ...
    const uploadUrlObj = uploadUrls[index];
    return async function() {
      if (new Date().getTime() - startUpload.getTime() > 10*60*1000) {
        // refresh upload tokens after 10 minutes (assuming token lifetime 15 min)
        startUpload = new Date();
        result = await withRetry(
          () => post(siteAPI.getImageUploadUrl, payload), shouldRetry);
        uploadUrls = result.data;
      }
