...
export async function uploadImages(imagesData, resourceID, resourceKey, siteAPI) {
  ...
  //upload the images
  let urls = [];
  await Queue(1, uploadUrlsAndData.map((item, index) => {
    const [imageDataObj, uploadUrlObj] = item;
    return async function() {
      logger.trace(`        importImages - ${resourceID} - ${resourceKey} uploading image ${index} of ${uploadUrlsAndData.length}`, uploadUrlObj.uploadUrl);
