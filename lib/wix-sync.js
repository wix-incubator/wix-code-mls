...
export async function uploadImages(imagesData, resourceID, resourceKey, siteAPI) {
  ...
  return Promise.all(uploadUrlsAndData.map((item, index) => {
    const [imageDataObj, uploadUrlObj] = item;
    return uploadImageViaUploadUrl(uploadUrlObj.uploadUrl, uploadUrlObj.uploadToken, imageDataObj.imageData,
