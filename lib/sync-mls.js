      ...
      if (status === 'need-update' || status === 'not-found') {
        tasks.push(async function () {
          if (tableConfig.syncImages) {
            ...
            let images = await syncImages(client, item, mediaCredentials, mediaConfig, siteAPI, tableConfig.resourceID, '_id', logger, startIndex + i);
            if (images.length > 0) {
              item.images = images;
...
module.exports.syncMLS = async function syncMLS(retsConfig, mediaCredentials, mediaConfig, siteAPI, tablesConfig) {
          ...
          if (tableConfig.sync) {
            try {
              ...
              if (itemsData.results.length > 0) {
                let itemBatches = batch(itemsData.results, 100);
  ...
  logger.log('  uploaded images:', syncImages.uploadedImages);
  logger.log('  failed uploads :', syncImages.failedUploadImages);
};
