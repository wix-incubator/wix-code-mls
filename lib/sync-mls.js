    ...
    if (tablesConfig[i].sync && !clearedCollections.find(_ => _ === tablesConfig[i].wixCollection)) {
      ...
      try {
        ...
        do {
          ...
          if (clearResult && clearResult.staleItems && clearResult.itemsRemoved) {
            ...
            logger.trace(`  error in clear items ${clearResult}`);
 ...
 */
module.exports.syncMLS = async function syncMLS(retsConfig, siteAPI, tablesConfig) {
  const logger = Logger;
