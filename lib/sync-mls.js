...
const clearStaleItems2 = wixSync.clearStaleItems2;
...
async function syncTable(retsConfig, mediaCredentials, mediaConfig, siteAPI, tableConfig, logger) {
  return new Promise(function(resolve) {
...
async function runClearStaleItems(tablesConfig, logger, siteAPI) {
  ...
  for (let i = 0; i < tablesConfig.length; i++) {
    if (tablesConfig[i].sync && !clearedCollections.find(_ => _ === tablesConfig[i].wixCollection)) {
      ...
      try {
        let pendingItems = 1;
        do {
          let clearResult = await clearStaleItems2(siteAPI, tablesConfig[i].wixCollection);
          pendingItems = clearResult.staleItems;
          logger.trace(`  cleared ${clearResult.itemsRemoved} items, pending ${clearResult.staleItems}, errors: ${clearResult.errors}`);
          await checkSleep();
...
module.exports.syncMLS = async function syncMLS(retsConfig, mediaCredentials, mediaConfig, siteAPI, tablesConfig) {
  ...
  for (let i=0; i < tablesConfig.length; i++) {
    if (tablesConfig[i].sync) {
      await syncTable(retsConfig, mediaCredentials, mediaConfig, siteAPI, tablesConfig[i], logger);
      await checkSleep();
    }
  }

  await runClearStaleItems(tablesConfig, logger, siteAPI);
