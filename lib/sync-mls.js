      ...
      if (status === 'need-update' || status === 'not-found' || (status === 'ok-no-images' && tableConfig.syncImages) || force) {
        ...
        tasks.push(async function () {
          if (tableConfig.syncImages) {
            let {images, noImagesReplyCode} = await linkToImages(client, item, tableConfig.resourceID, '_id', startIndex + i, tableConfig.overrideGetImagesUrl, tableConfig.alternateImageObjectType);
