      ...
      if (status === 'need-update' || status === 'not-found' || (status === 'ok-no-images' && tableConfig.syncImages) || force) {
        ...
        tasks.push(async function () {
          if (tableConfig.syncImages) {
            let {images, noImagesReplyCode} = await linkToImages(client, item, tableConfig.resourceID, '_id', startIndex + i, tableConfig.overrideGetImagesUrl);
      ...
      try {
        ...
        do {
          ...
          if (queryResult.results.length > 0) {
            let itemBatches = batch(queryResult.results, options.batchSize || batchSize);
            for (let i = 0; i < itemBatches.length; i++) {
              ...
              await handleBatch(itemBatches[i], client, siteAPI, tableConfig, logger, i*batchSize, options.force);
