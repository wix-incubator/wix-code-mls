...
import {uploadImage} from './wix-sync';
...
export default async function getImageUrls(client, item, resourceID, keyField, itemIndex,
  ...
  if (overrideGetImagesUrl)
    ...
    return importImages(client, item, resourceID, keyField, itemIndex, siteAPI, checkSleep);
  else
...
async function importImages(client, item, resourceID, keyField, itemIndex, siteAPI, checkSleep) {
  let resourceKey = item[keyField];
  let objectType = 'Photo';
  logger.log('      importImages', resourceID, objectType, resourceKey, 'item', itemIndex);
  let photoResults;
  let images = [];
  ...
  for (let imageIndex = 0; imageIndex < photoResults.objects.length; imageIndex++) {
    let obj = photoResults.objects[imageIndex];
    ...
    else if (obj.headerInfo && obj.headerInfo.contentType && obj.data) {
      // a photo
      try {
        let imageData = obj.data;
        let mimeType = obj.headerInfo.contentType;
        await checkSleep(logger);
        logger.trace(`        importImages - uploading image ${resourceID} ${resourceKey} item ${itemIndex} image ${imageIndex}/${photoResults.objects.length-1}`);
        let url = await uploadImage(imageData, mimeType, resourceID, imageIndex, resourceKey, siteAPI);
        images.push({
          type: 'image',
          src: url
        });
        ...
        logger.warn(`        importImages - Failed to upload image for ${resourceID} ${resourceKey} ${imageIndex} with error ${e.message}. Skipping the image`);
      }
    }
  }
  logger.trace('        importImages - has ',(images.length===0)?'no':images.length,' image', resourceID, resourceKey);
