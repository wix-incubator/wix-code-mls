...
export default async function getImageUrls(client, item, resourceID, keyField, itemIndex,
  ...
  if (overrideGetImagesUrl)
    return runOverrideGetImageUrl(client, item, resourceID, keyField, itemIndex, siteAPI, overrideGetImagesUrl);
...
async function runOverrideGetImageUrl(client, item, resourceID, keyField, itemIndex, siteAPI, overrideGetImagesUrl) {
  ...
  const uploadImagesToWix = (imagesData) => {
    return uploadImages(imagesData, resourceID, resourceKey, siteAPI)
  };

  try {
    let imageUrls = await hook(client, item, resourceID, keyField, itemIndex, logger, uploadImagesToWix);
