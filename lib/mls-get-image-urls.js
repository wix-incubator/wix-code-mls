...
export default async function getImageUrls(client, item, resourceID, keyField, itemIndex,
                                           overrideGetImagesUrl, alternateImageObjectType, importImagesToWixMedia, siteAPI) {
  if (overrideGetImagesUrl)
    ...
    return importImages(client, item, resourceID, keyField, itemIndex, siteAPI);
  else
...
async function importImages(client, item, resourceID, keyField, itemIndex, siteAPI) {
  ...
  for (let i = 0; i < photoResults.objects.length; i++) {
    let obj = photoResults.objects[i];
    if (obj.type === 'headerInfo') {
      ...
      let url = await uploadImage(imageData, mimeType, resourceID, itemIndex, resourceKey, siteAPI);
      images.push({
        type: 'image',
        src: url
      });
